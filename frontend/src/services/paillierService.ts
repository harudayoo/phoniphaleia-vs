...
export function encryptPaillierVote(publicKeyJson: string, voteValue: number | string | bigint): string {
  // Parse the public key JSON (should contain 'n' and optionally 'g')
  const pub = JSON.parse(publicKeyJson);
  const n = BigInt(pub.n);
  // Use g = n + 1 if not provided (standard Paillier)
  const g = pub.g ? BigInt(pub.g) : n + 1n;
